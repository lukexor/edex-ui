...
window.onerror = (msg, path, line, col, error) => {
  document.getElementById(
    "boot_screen"
  ).innerHTML += `${error} :  ${msg}<br/>==> at ${path}  ${line}:${col}`;
};
...
window._loadTheme = (theme) => {
  ...
  let mainFont = new FontFace(
    ...
    `url("${path
      .join(
        fontsDir,
        theme.cssvars.font_main.toLowerCase().replace(/ /g, "_") + ".woff2"
      )
      .replace(/\\/g, "/")}")`
  );
  let lightFont = new FontFace(
    ...
    `url("${path
      .join(
        fontsDir,
        theme.cssvars.font_main_light.toLowerCase().replace(/ /g, "_") +
          ".woff2"
      )
      .replace(/\\/g, "/")}")`
  );
  let termFont = new FontFace(
    ...
    `url("${path
      .join(
        fontsDir,
        theme.terminal.fontFamily.toLowerCase().replace(/ /g, "_") + ".woff2"
      )
      .replace(/\\/g, "/")}")`
  );
  ...
  document.querySelector("head").innerHTML += `<style class="theming">
    :root {
        ...
        --font_main_light: "${window._purifyCSS(
          theme.cssvars.font_main_light
        )}";
    ...
    body {
        ...
        cursor: ${
          window.settings.nocursorOverride || window.settings.nocursor
            ? "none"
            : "default"
        } !important;
    }
    ...
    {},
    {
      ...
      get: (target, prop, receiver) => {
        return function (...args) {
          let callback =
            typeof args[args.length - 1] === "function" ? true : false;
...
async function displayTitleScreen() {
  ...
  title.setAttribute(
    "style",
    `border: 5px solid rgb(${window.theme.r}, ${window.theme.g}, ${window.theme.b});`
  );
...
async function initUI() {
  ...
  document
    ...
    .setAttribute("class", "animation_state_1");
  ...
  document
    .getElementById("keyboard")
    .setAttribute("class", "animation_state_1 animation_state_2");
  ...
  window.term[0].term.writeln(
    "\033[1m" +
      `Welcome to eDEX-UI v${electron.remote.app.getVersion()} - Electron v${
...
window.remakeKeyboard = (layout) => {
  ...
  window.keyboard = new Keyboard({
    layout: path.join(
      keyboardsDir,
      layout + ".json" || settings.keyboard + ".json"
    ),
    container: "keyboard",
  });
...
window.focusShellTab = (number) => {
  ...
  if (number !== window.currentTerm && window.term[number]) {
    ...
    document
      .querySelectorAll(
        `ul#main_shell_tabs > li:not(:nth-child(${number + 1}))`
      )
      .forEach((e) => {
        e.setAttribute("class", "");
      });
    document
      .getElementById("shell_tab" + number)
      .setAttribute("class", "active");

    document
      .querySelectorAll(
        `div#main_shell_innercontainer > pre:not(:nth-child(${number + 1}))`
      )
      .forEach((e) => {
        e.setAttribute("class", "");
      });
    document
      .getElementById("terminal" + number)
      .setAttribute("class", "active");
    ...
    typeof window.term[number] !== "object"
  ) {
    window.term[number] = null;
    ...
    ipc.once("ttyspawn-reply", (e, r) => {
      if (r.startsWith("ERROR")) {
        ...
        window.term[number].onprocesschange = (p) => {
          document.getElementById("shell_tab" + number).innerHTML = `<p>#${
            number + 1
          } - ${p}</p>`;
        };
...
window.openSettings = async () => {
  ...
  nets.forEach((net) => {
    if (net.iface !== window.mods.netstat.iface)
  ...
  new Modal(
    {
      ...
      html: `<table id="settingsEditor">
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="settingsEditor-shell" value="${
                    ...
                    <tr>
                        ...
                        <td><input type="text" id="settingsEditor-cwd" value="${
                          window.settings.cwd
                        }"></td>
                    </tr>
                    <tr>
                        ...
                        <td><input type="text" id="settingsEditor-env" value="${
                          window.settings.env
                        }"></td>
                    </tr>
                    <tr>
                        ...
                        <td><input type="text" id="settingsEditor-username" value="${
                    ...
                    <tr>
                        ...
                        <td><input type="number" id="settingsEditor-termFontSize" value="${
                          window.settings.termFontSize
                        }"></td>
                    </tr>
                    <tr>
                        <td>audio</td>
                    ...
                    <tr>
                        ...
                        <td><input type="number" id="settingsEditor-audioVolume" value="${
                          window.settings.audioVolume || "1.0"
                        }"></td>
                    </tr>
                    <tr>
                        ...
                        <td><select id="settingsEditor-disableFeedbackAudio">
                            ...
                            <option>${!window.settings
                              .disableFeedbackAudio}</option>
                        </select></td>
                    </tr>
                    <tr>
                        <td>port</td>
                        <td>Local port to use for UI-shell connection</td>
                        <td><input type="number" id="settingsEditor-port" value="${
                    ...
                    <tr>
                        ...
                        <td><select id="settingsEditor-monitor">
                            ${
                              typeof window.settings.monitor !== "undefined"
                                ? "<option>" +
                                  window.settings.monitor +
                                  "</option>"
                                : ""
                            }
                            ${monitors}
                    ...
                    <tr>
                        <td>nointro</td>
                        <td>Skip the intro boot log and logo${
                          window.settings.nointroOverride
                    ...
                    <tr>
                        ...
                        <td>Hide the mouse cursor${
                          window.settings.nocursorOverride
                            ? " (Currently overridden by CLI flag)"
                            : ""
                        }</td>
                    ...
                    <tr>
                        ...
                        <td><select id="settingsEditor-keepGeometry">
                            <option>${
                              window.settings.keepGeometry === false
                                ? "false"
                                : "true"
                            }</option>
                            <option>${
                    ...
                    <tr>
                        ...
                        <td><select id="settingsEditor-excludeThreadsFromToplist">
                            ...
                            <option>${!window.settings
                              .excludeThreadsFromToplist}</option>
                        </select></td>
                    </tr>
                    <tr>
                        <td>hideDotfiles</td>
                    ...
                    <tr>
                        ...
                        <td><select id="settingsEditor-experimentalGlobeFeatures">
                            ...
                            <option>${!window.settings
                              .experimentalGlobeFeatures}</option>
                        </select></td>
                    </tr>
                    <tr>
                        ...
                        <td><select id="settingsEditor-experimentalFeatures">
                            <option>${
                              window.settings.experimentalFeatures
                            }</option>
                            <option>${!window.settings
                              .experimentalFeatures}</option>
                        </select></td>
                    </tr>
                </table>
      ...
      buttons: [
        {
          ...
          action: `electron.shell.openItem('${settingsFile}');electronWin.minimize();`,
        },
        { label: "Save to Disk", action: "window.writeSettingsFile()" },
        { label: "Reload UI", action: "window.location.reload(true);" },
        {
          label: "Restart eDEX",
          action: "electron.remote.app.relaunch();electron.remote.app.quit();",
        },
      ],
    },
    () => {
...
window.writeSettingsFile = () => {
  window.settings = {
    ...
    termFontSize: Number(
      document.getElementById("settingsEditor-termFontSize").value
    ),
    audio: document.getElementById("settingsEditor-audio").value === "true",
    audioVolume: Number(
      document.getElementById("settingsEditor-audioVolume").value
    ),
    ...
    clockHours: Number(
      document.getElementById("settingsEditor-clockHours").value
    ),
    ...
    nocursor:
      document.getElementById("settingsEditor-nocursor").value === "true",
    iface: document.getElementById("settingsEditor-iface").value,
    allowWindowed:
      document.getElementById("settingsEditor-allowWindowed").value === "true",
    keepGeometry:
      document.getElementById("settingsEditor-keepGeometry").value === "true",
    excludeThreadsFromToplist:
      document.getElementById("settingsEditor-excludeThreadsFromToplist")
        .value === "true",
    hideDotfiles:
      document.getElementById("settingsEditor-hideDotfiles").value === "true",
    fsListView:
      document.getElementById("settingsEditor-fsListView").value === "true",
    experimentalGlobeFeatures:
      document.getElementById("settingsEditor-experimentalGlobeFeatures")
        .value === "true",
    experimentalFeatures:
      document.getElementById("settingsEditor-experimentalFeatures").value ===
      "true",
  };
...
window.toggleFullScreen = () => {
  ...
  fs.writeFileSync(
    lastWindowStateFile,
    JSON.stringify(window.lastWindowState, "", 4)
  );
};
...
window.openShortcutsHelp = () => {
  ...
  const shortcutsDefinition = {
    ...
    KB_PASSMODE:
      'Toggle the on-screen keyboard\'s "Password Mode", which allows you to safely<br>type sensitive information even if your screen might be recorded (disable visual input feedback).',
  ...
  window.shortcuts
    ...
    .forEach((cut) => {
      ...
      appList += `<tr>
                        ...
                        <td><input disabled type="text" maxlength=25 value="${
                          cut.trigger
                        }"></td>
  ...
  window.shortcuts
    ...
    .forEach((cut) => {
      customList += `<tr>
                            ...
                            <td>
                                <input disabled type="text" placeholder="Run terminal command..." value="${
                                  cut.action
                                }">
                                <input disabled type="checkbox" name="shortcutsHelpNew_Enter" ${
                                  cut.linebreak ? "checked" : ""
                                }>
      ...
      buttons: [
        {
          ...
          action: `electron.shell.openItem('${shortcutsFile}');electronWin.minimize();`,
        },
        { label: "Reload UI", action: "window.location.reload(true);" },
      ],
    },
