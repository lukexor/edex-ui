  ...
  constructor(opts) {
    ...
    this.readFS = async (dir) => {
      ...
      if (this._noTracking) {
        ...
        ).innerText = "FILESYSTEM - TRACKING FAILED, RUNNING DETACHED FROM TTY";
      }
      ...
      await new Promise((resolve, reject) => {
        ...
        content.forEach(async (file, i) => {
          let fstat = await this._asyncFSwrapper
            .lstat(path.join(tcwd, file))
            .catch((e) => {
              if (
    ...
    this.render = async (originBlockList, isDiskView) => {
      ...
      blockList.forEach((e, blockIndex) => {
        ...
        if (!this._noTracking) {
          if (e.type === "dir" || e.type.endsWith("Dir")) {
            ...
            if (process.platform === "win32") {
              cmd = `window.term[window.currentTerm].writelr("${e.path.replace(
                /\\/g,
                ""
              )}")`;
            } else {
              cmd = `window.term[window.currentTerm].writelr("cd \\"${e.path.replace(
                /\\/g,
                ""
              )}\\"")`;
            }
        ...
        filesDOM += `<div class="fs_disp_${
          e.type
        }${hidden} animationWait" onclick='${cmdPrefix + cmd + cmdSuffix}'>
                                <svg viewBox="0 0 ${icon.width} ${
          icon.height
        }" fill="${this.iconcolor}">
                                    ${icon.svg}
                                </svg>
      ...
      if (this.filesContainer.getAttribute("class").endsWith("disks")) {
        document
          ...
          .setAttribute("onclick", "window.fsDisp.render(window.fsDisp.cwd)");
      } else {
    ...
    this.renderDiskUsage = async (fsBlock) => {
      ...
      let displayMount =
        fsBlock.mount.length < 18
          ? fsBlock.mount
          : "..." + splitter + fsBlock.mount.split(splitter).pop();

      // See #226
      if (!isNaN(fsBlock.use)) {
        this.space_bar.text.innerHTML = `Mount <strong>${displayMount}</strong> used <strong>${Math.round(
          fsBlock.use
        )}%</strong>`;
    ...
    this.openMedia = (name, path, type) => {
      ...
      switch (type || block.type) {
        case "image":
          html = `<img class="fsDisp_mediaDisp" src="${window._encodePathURI(
            path || block.path
        ...
        default:
          throw new Error(
            "fsDisp media displayer: unknown type " + (type || block.type)
          );
      }
