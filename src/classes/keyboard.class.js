  ...
  constructor(opts) {
    ...
    Object.keys(layout).forEach((row) => {
      this.container.innerHTML +=
        `<div class="keyboard_row" id="` + row + `"></div>`;
      layout[row].forEach((keyObj) => {
        ...
        Object.keys(keyObj).forEach((property) => {
          for (let i = 1; i < this.ctrlseq.length; i++) {
            keyObj[property] = keyObj[property].replace(
              "~~~CTRLSEQ" + i + "~~~",
              this.ctrlseq[i]
            );
          }
    ...
    let dropKeyTouchHandler = (e) => {
      ...
      for (let i = 0; i < e.changedTouches.length; i++) {
        ...
        if (key.getAttribute("class").startsWith("keyboard_key")) {
          key.setAttribute(
            "class",
            key.getAttribute("class").replace("active", "")
          );
          key.onmouseup({
            preventDefault: () => {
              return true;
    ...
    let findKey = (e) => {
      ...
      let key = document.querySelector(
        'div.keyboard_key[data-cmd="' + physkey + '"]'
      );
      if (key === null)
        key = document.querySelector(
          'div.keyboard_key[data-shift_cmd="' + physkey + '"]'
        );
  ...
  pressKey(key) {
    ...
    if (window.keyboard.linkedToTerm) {
      ...
      if (typeof document.activeElement.value !== "undefined") {
        switch (cmd) {
          case "":
            document.activeElement.value = document.activeElement.value.slice(
              0,
              -1
            );
            isDelete = true;
      ...
      document.activeElement.dispatchEvent(
        new CustomEvent("input", { detail: isDelete ? "delete" : "insert" })
      );
