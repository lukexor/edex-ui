class FuzzyFinder {
    ...
    this.input.addEventListener("keydown", (e) => {
      ...
      switch (e.key) {
        ...
        case "ArrowDown":
          ...
          next = document.getElementById(`fuzzyFinderMatch-${selected + 1}`)
            ? selected + 1
            : 0;
  ...
  search(text) {
    ...
    results.sort((a, b) => {
      if (
        a.name.toLowerCase().startsWith(text.toLowerCase()) &&
        !b.name.toLowerCase().startsWith(text.toLowerCase())
      ) {
        return -1;
      } else if (
        !a.name.toLowerCase().startsWith(text.toLowerCase()) &&
    ...
    results.forEach((file, i) => {
      html += `<li id="fuzzyFinderMatch-${i}" class="${
        i === 0 ? "fuzzyFinderMatchSelected" : ""
      }" onclick="document.querySelector('li.fuzzyFinderMatchSelected').removeAttribute('class');document.getElementById('fuzzyFinderMatch-${i}').setAttribute('class', 'fuzzyFinderMatchSelected')">${
        file.name
      }</li>`;
    });
    if (results.length !== 5) {
