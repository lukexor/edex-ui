...
class Modal {
  constructor(options, onclose) {
    ...
    let DOMstring = `<div id="modal_${this.id}" class="${
      this.classes
    }" style="z-index:${
      zindex + Object.keys(window.modals).length
    };" augmented-ui="${augs.join(" ")} exe">
            <h1>${this.title}</h1>
            ${
              this.type === "custom"
    ...
    let dragTarget = document.querySelector(
      `div#modal_${this.id} > h1:first-child`
    );
    ...
    function modalMousemoveHandler(e) {
      draggedModal.posX =
        draggedModal.posX + (e.clientX - draggedModal.lastMouseX);
      draggedModal.posY =
        draggedModal.posY + (e.clientY - draggedModal.lastMouseY);
      draggedModal.lastMouseX = e.clientX;
      draggedModal.lastMouseY = e.clientY;
    ...
    function modalTouchmoveHandler(e) {
      draggedModal.posX =
        draggedModal.posX +
        (e.changedTouches[0].clientX - draggedModal.lastMouseX);
      draggedModal.posY =
        draggedModal.posY +
        (e.changedTouches[0].clientY - draggedModal.lastMouseY);
      draggedModal.lastMouseX = e.changedTouches[0].clientX;
      draggedModal.lastMouseY = e.changedTouches[0].clientY;
