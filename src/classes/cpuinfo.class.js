class Cpuinfo {
  constructor(parentId) {
    ...
    window.si.cpu().then((data) => {
      ...
      for (var i = 0; i < 2; i++) {
        this.charts[i].streamTo(
          document.getElementById(`mod_cpuinfo_canvas_${i}`),
          500
        );
      }
  ...
  updateCPUspeed() {
    window.si.cpuCurrentspeed().then((data) => {
      try {
        ...
        ).innerText = `${data.min}GHz`;
        document.getElementById(
          "mod_cpuinfo_speed_max"
        ).innerText = `${data.max}GHz`;
      } catch (e) {
        // See above notice
      }
    });
