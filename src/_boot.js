...
process.on("uncaughtException", (e) => {
  ...
  dialog.showErrorBox(
    "eDEX-UI crashed",
    e.message || "Cannot retrieve error message."
  );
  if (tty) {
    tty.close();
  }
...
signale.info(
  `With Node ${process.versions.node} and Electron ${process.versions.electron}`
);
...
if (!gotLock) {
  signale.fatal(
    "Error: Another instance of eDEX is already running. Cannot proceed."
  );
  app.exit(1);
}
...
const lastWindowStateFile = path.join(
  electron.app.getPath("userData"),
...
fs.readdirSync(innerThemesDir).forEach((e) => {
  fs.writeFileSync(
    path.join(themesDir, e),
    fs.readFileSync(path.join(innerThemesDir, e), { encoding: "utf-8" })
  );
});
try {
...
fs.readdirSync(innerFontsDir).forEach((e) => {
  fs.writeFileSync(
    path.join(fontsDir, e),
    fs.readFileSync(path.join(innerFontsDir, e))
  );
});
...
var versionHistory = fs.existsSync(versionHistoryPath)
  ? require(versionHistoryPath)
  : {};
...
fs.writeFileSync(versionHistoryPath, JSON.stringify(versionHistory, 0, 2), {
  encoding: "utf-8",
});
...
function createWindow(settings) {
  ...
  if (!isNaN(settings.monitor)) {
    display =
      electron.screen.getAllDisplays()[settings.monitor] ||
...
app.on("ready", async () => {
  ...
  if (!require("fs").existsSync(settings.cwd))
    throw new Error("Configured cwd path does not exist.");

  // See #366
  let cleanEnv = await require("shell-env")(settings.shell.split(" ")[0]).catch(
    (e) => {
      throw e;
    }
  );
  ...
  signale.pending(
    `Creating new terminal process on port ${settings.port || "3000"}`
  );
  tty = new Terminal({
    role: "server",
